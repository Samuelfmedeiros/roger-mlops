...
| Same `Resuming from step N` banner repeating | the banner itself (it prints every resurrection) | resume count per step (`tatu/night_watch.py::crash_loop_watch`) | ≥3 on the same step → host fault, not process fault; stop relaunching |
...
| A report says `step 999`, log says `step 60` | the report, always | the critic compares claimed vs parsed (`roger_tatu.py`) | claims > log → phantom self-report → escalate |
